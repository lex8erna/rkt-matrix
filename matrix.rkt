...
;; leading-co-eff : L-Vector -> (Union false Num)
;; PRE:   true
;; POST:  Returns the first non-zero entry in an L-Vector.
;;        If none, returns false.
(define (leading-co-eff a)
  (cond [(empty? a) false]
        [(not (zero? (first a))) (first a)]
        [else (leading-co-eff (rest a))]))

;; leading-index : L-Vector -> (Union false Nat)
;; PRE:   true
;; POST:  Returns the index of the first non-zero entry in an L-Vector.
;;        If none, returns false.
(define (leading-index a)
  (define (search v i)
    (cond [(empty? v) false]
          [(not (zero? (first v))) i]
          [else (search (rest v) (add1 i))]))
  (search a 0))
...
;; Quiet-ERO-swap! : Matrix Nat Nat -> Void
;; PRE:   row1, row2 are within range.
;; POST:  Performs the Elementary Row Operation of row exchange on Matrix A,
;;        for row1 <-> row2.
(define (Quiet-ERO-swap! A row1 row2)
  (define EM (EM-swap (matrix-entries A) row1 row2))
  (set-matrix-entries! A (matrix-multiply EM (matrix-entries A)))
  (set-matrix-reverse! A (matrix-multiply EM (matrix-reverse A))))
...
;;        Matrix A, for scalar*row. The operation and results are displayed.
...
;; Quiet-ERO-mult! : Matrix Nat Num -> Void
;; PRE:   row is within range.
;;        scalar is non-zero.
;; POST:  Performs the Elementary Row Operation of scalar multiplication on
;;        Matrix A, for scalar*row.
(define (Quiet-ERO-mult! A row scalar)
  (define EM (EM-mult (matrix-entries A) row scalar))
  (set-matrix-entries! A (matrix-multiply EM (matrix-entries A)))
  (set-matrix-reverse! A (matrix-multiply EM (matrix-reverse A))))
...
;;        for row1+scalar*row2. The operation and results are displayed.
...
;; Quiet-ERO-plus! : Matrix Nat Nat Num -> Void
;; PRE:   row1, row2 are within range.
;;        scalar is non-zero.
;; POST:  Performs the Elementary Row Operation of row addition on Matrix A,
;;        for row1+scalar*row2. The operation and results are displayed.
(define (Quiet-ERO-plus! A row1 row2 scalar)
  (define EM (EM-plus (matrix-entries A) row1 row2 scalar))
  (set-matrix-entries! A (matrix-multiply EM (matrix-entries A)))
  (set-matrix-reverse! A (matrix-multiply EM (matrix-reverse A))))
...
(define mlt ERO-mult!)
(define pls ERO-plus!)
(define qswp Quiet-ERO-swap!)
(define qmlt Quiet-ERO-mult!)
(define qpls Quiet-ERO-plus!)
    ...
    (cond [(= n rows)
          ...
          [else
           ...
           (cond [search-result
                 ...
                 [else
                  (printf "No suitable rows found for Column ~a\n" (add1 n))])
...
;; quiet-row-reduce! : Matrix -> Void
;; PRE:   true
;; POST:  Row-reduces Matrix A. (Quiet)
;; Notes: For the algorithm below, n is used to record the number of
;;        rows.
(define (quiet-row-reduce! A)
  (define rows (length (matrix-entries A)))
  ;; row-search : Nat Nat L-Matrix -> (Union false Nat)
  ;; PRE:   true
  ;; POST:  Returns the index of the first suitable row for which the n-th
  ...
  (define (row-zero n i origin entries)
    (cond [(empty? entries) void]
          ...
          [else
           (define elim-co-eff
             (- (list-ref (first entries) n)))
           (qpls A (add1 i) (add1 origin) elim-co-eff)
           (row-zero n (add1 i) origin (rest entries))]))
  ;; row-swap : Nat Nat -> Void
  ;; PRE:   true
  ;; POST:  Performs row exchange if necessary.
...
(define QRREF! quiet-row-reduce!)

;; REF? : Matrix -> Boolean
;; PRE:   true
;; POST:  Returns true if the Matrix A is in Row Echelon Form.
;; Notes: A is in Row Echelon Form if: 
;;          1. All non-zero rows are above all zero rows.
;;          2. Leading co-efficients are strictly to the right of any leading 
;;             co-efficients in rows above it.
(define (REF? A)
  (define entries (matrix-entries A))
  (define (criterion-search last-lead-index zero-row entries)
    (cond [(empty? entries) true]
          [else 
           (define lead-index (leading-index (first entries)))
           (cond [(and lead-index 
                       (or zero-row
                           (<= lead-index last-lead-index)))
                  false]
                 [(false? lead-index)
                  (criterion-search +inf.0 true (rest entries))]
                 [else
                  (criterion-search lead-index zero-row (rest entries))])]))
  (criterion-search -inf.0 false entries))

;; RREF? : Matrix -> Boolean
;; PRE:   true
;; POST:  Returns true if the Matrix A is in Reduced Row Echelon Form.
;; Notes: A is in Reduced Row Echelon Form if:
;;          1. A is in Row Echelon Form
;;          2. All leading co-efficients are 1
;;          3. All leading 1's are the only non-zero entries in the column.
(define (RREF? A)
  (define entries (matrix-entries A))
  (define entries-T (transpose entries))
  (define (loneliness? v n i)
    (cond [(empty? v) true]
          [(or (= n i)
               (zero? (first v)))
           (loneliness? (rest v) n (add1 i))]
          [else false]))
  (define (valid-row? v v-T)
    (define result (leading-co-eff v))
    (or (not result)
        (and (= result 1)
             (loneliness? v-T (leading-index v) 0))))
  (define (criterion-search A A-T)
    (cond [(empty? A) true]
          [(valid-row? (first A) (first A-T))
           (criterion-search (rest A) (rest A-T))]
          [else false]))
  (and (REF? A)
       (criterion-search entries entries-T)))

;; rank : Matrix -> Nat
;; PRE:   true
;; POST:  Returns the rank of Matrix A.
(define (rank A)
  (define RREF
    (cond [(RREF? A) (matrix-entries A)]
          [else
           (define R (new-matrix (matrix-entries A)))
           (QRREF! R)
           (matrix-entries R)]))
